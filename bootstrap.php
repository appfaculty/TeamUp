...
function get_favicon(string $entry): string
{
    ...
    if (!empty($teamupconfig->favicon)) {
        $favicon = 'data:image/x-icon;base64,' . $teamupconfig->favicon;
    } else {
        // Default teamup favicon.
...
{
    $teamupconfig = get_config('local_teamup');
    if (!empty($teamupconfig->logo)) {
        $logo = 'data:image/x-icon;base64,' . $teamupconfig->logo;
    } else {
        // Default teamup logo.
        $manifest = getManifest();
        $src = isDev($entry)
